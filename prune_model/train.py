...
def parse_args():
    ...
    parser.add_argument('--tune', default='', type=str, metavar='PATH',
                    help='fine tune from prune model')
    parser.add_argument('--tune_epochs', type=int, default=50, metavar='N',
...
def train():
    ...
    for epoch in range(args.start_epoch, max_epoch):
        ...
        for iter_i, (images, targets) in enumerate(dataloader):
            ...
            # multi-scale trick
            if iter_i % 2 == 0 and iter_i > 0 and args.multi_scale:
...
#                 print(train_size)
            ...
            # forward and loss
#             print(len(model(images, target=targets)))
#             print(model(images, target=targets))
            ...
            # 好像有一些梯度为0？然后打印出来的值为None？？？
            # todo: 查看一下batchnorm的原理
            if args.sr:
                for m in model.modules():
                    # slim里面有两层不参与梯度计算，所以没有grad，需要注意屏蔽！！！
                    if isinstance(m, nn.BatchNorm2d) and m.weight.grad != None:
                        # 这里做了增强，给BN层增加了一个惩罚项
                        # torch.sign 符号函数，就是L1正则化
                        # m.weight.data是惩罚的尺度因子
        ...
        if (epoch + 1) % 1 == 0:
            if not use_horovod or (use_horovod and hvd.rank() == 0):
                ...
                #             )
                # 保存权重和模型
                torch.save(model, os.path.join(path_to_save, 
                            args.version + '_' + repr(epoch + 1) + '.pth')
                            )
